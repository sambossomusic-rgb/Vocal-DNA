...
export interface VoiceProfileOptions {
  /**
   * Which repertoire statuses to analyse (V4 item 4). Defaults to the active
   * repertoire — Regular + Occasional — so Unexplored songs never skew the
   * profile; callers can opt Learning in.
   */
  includeStatuses?: RepertoireStatus[];
}

const DEFAULT_ACTIVE: RepertoireStatus[] = ['regular', 'occasional'];
 ...
 * acoustic measurement. Key-change recommendations moved to
 * recommendationEngine (V4).
 */
export function computeVoiceProfile(
  ...
  const includeStatuses = new Set(options.includeStatuses ?? DEFAULT_ACTIVE);
  ...
  // --- Fatigue trend, most recent 20 rated songs chronologically ---
